...
func (ftp *FTP) AUTH_TLS() error {
	tempResponse, err := ftp.SendAndRead(AUTH_TLS_CMD)
	if err != nil {
		return err
	}
	...
	ftp.textprotoConn = textproto.NewConn(ftp.conn)

	tempResponse, err = ftp.SendAndRead(PBSZ_CMD)
	if err != nil {
		return err
	}

	if tempResponse.getFirstChar() == "2" {
		tempResponse, err = ftp.SendAndRead(PROT_CMD)
		if err != nil {
			return err
		}
	}
