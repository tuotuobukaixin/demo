...
func (d *dbBase) ReadBatch(q dbQuerier, qs *querySet, mi *modelInfo, cond *Condition, container interface{}, tz *time.Location, cols []string) (int64, error) {
	...
	query := fmt.Sprintf("SELECT %s FROM %s%s%s T0 %s%s%s", sels, Q, mi.table, Q, join, where, orderBy)

	query = tables.getLimitSql(query, offset, rlimit)
...
func (d *dbBase) ReadValues(q dbQuerier, qs *querySet, mi *modelInfo, cond *Condition, exprs []string, container interface{}, tz *time.Location) (int64, error) {
	...
	//query := fmt.Sprintf("SELECT %s FROM %s%s%s T0 %s%s%s%s", sels, Q, mi.table, Q, join, where, orderBy, limit)
	query := fmt.Sprintf("SELECT %s FROM %s%s%s T0 %s%s%s", sels, Q, mi.table, Q, join, where, orderBy)
	query = tables.getLimitSql(query, qs.offset, qs.limit)
...
/*func (d *dbBase) SuitForKeyWords(fi *fieldInfo) {

}*/

func (d *dbBase) GetLimitSql(query string, offset, limit int64) (sql string) {
	if limit == 0 {
	...
	if limit < 0 {
		...
		if offset > 0 {
			maxLimit := d.MaxLimit()
			if maxLimit == 0 {
				sql = fmt.Sprintf("%s OFFSET %d", query, offset)
			} else {
				sql = fmt.Sprintf("%s LIMIT %d OFFSET %d", query, maxLimit, offset)
			}
		}
	} else if offset <= 0 {
		sql = fmt.Sprintf("%s LIMIT %d", query, limit)
	} else {
		sql = fmt.Sprintf("%s LIMIT %d OFFSET %d", query, limit, offset)
	}
	return
}

func (d *dbBase) SuitForNull(values []interface{}) {
