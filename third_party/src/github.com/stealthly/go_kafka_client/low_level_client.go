...
func (this *SaramaClient) Fetch(topic string, partition int32, offset int64) ([]*Message, error) {
	...
	if err != nil {
		this.client.RefreshMetadata(topic)
		if err.Error() == "EOF" {
			Debugf(this, "Get a EOF response error, try fetching again. topic=%s, partition=%d :%s", topic, partition)
	...
	if response != nil {
		Debugf(this, "Processing fetch response: topic=%s, partition=%d, offset=%d", topic, partition, offset)
