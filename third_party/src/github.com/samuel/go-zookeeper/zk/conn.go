	...
	for {
		...
		if c.serverIndex == c.lastServerIndex {
			...
			case <-time.After(3 * time.Second):
				// pass
				c.logger.Printf("Failed to connect ,time.After(time.Second).")
			case <-c.shouldQuit:
				c.setState(StateDisconnected)
				c.flushUnsentRequests(ErrClosing)
				c.logger.Printf("Failed to connect ,shouldQuit.")
				return ErrClosing
			}
		...
		if err == nil {
			...
			c.logger.Printf("success connect.")
			return nil
		}
...
func (c *Conn) loop() {
	for {
		...
		c.setState(StateDisconnected)
		c.logger.Printf("setState StateDisconnected")
		// Yeesh
		if err != io.EOF && err != ErrSessionExpired && !strings.Contains(err.Error(), "use of closed network connection") {
			c.logger.Printf(err.Error())
		}

		c.logger.Printf("wait shouldQuit")
		select {
		case <-c.shouldQuit:
			c.logger.Printf("return shouldQuit")
			c.flushRequests(ErrClosing)
			return
		default:
		}

		c.logger.Printf("flushRequests err %v", err)
		if err != ErrSessionExpired {
			err = ErrConnectionClosed
		}
		c.flushRequests(err)

		c.logger.Printf("reconnectDelay %d", c.reconnectDelay)
		if c.reconnectDelay > 0 {
			select {
			case <-c.shouldQuit:

				c.logger.Printf("retry reconnectDelay shouldQuit")
				return
			case <-time.After(c.reconnectDelay):

				c.logger.Printf("retry reconnectDelay %d", c.reconnectDelay)
			}
		}
	}
}
...
func (c *Conn) authenticate() error {
	...
	if err != nil {
		...
		c.logger.Printf("Failed to authenticate StateExpired because of SetReadDeadline: %s", err.Error())
	...
	if r.SessionID == 0 {
		...
		c.lastZxid = 0
		c.setState(StateExpired)
		c.logger.Printf("Failed to authenticate StateExpired because of r.SessionID is 0")
		return ErrSessionExpired
	}
